...
def get_and_save_data(granularity, months):
    ...
    df = pd.DataFrame(price_data, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'usdVolume'])
     # timestamp를 명시적으로 int로 변환
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')

    # UTC to KST (Korea Standard Time)
    ...
    df = df.sort_index(ascending=False)  # 최근 시간이 가장 상단에 오도록 정렬
    
    for col in ['open', 'high', 'low', 'close', 'volume', 'usdVolume']:
...
if __name__ == "__main__":
    ...
    if main_menu == "데이터 로드":
        if st.sidebar.button("데이터 로드", key="load_data_button"):
            try:
                with st.spinner("데이터를 불러오는 중..."):
                    ...
                    for granularity, months in timeframes.items():
                        df = get_and_save_data(granularity, months)
                        if df is not None and not df.empty:
                            st.session_state[f'data_{granularity}'] = df
                        else:
                            st.sidebar.warning(f"{granularity}분 데이터 로드 실패")
                    
                    st.sidebar.success("모든 시간프레임 데이터 로드 완료!")
            except Exception as e:
                st.sidebar.error(f"데이터 로드 중 오류 발생: {str(e)}")

        # 데이터 표시
        if 'data_3600' in st.session_state:
            st.write("1시간봉 데이터 샘플 (최근 순, 한국 시간):")
            st.write(st.session_state['data_3600'].head())
        else:
        ...
        months = months_map[granularity]

        try:
            if f'data_{granularity}' in st.session_state:
                df = st.session_state[f'data_{granularity}']
            else:
                df = get_and_save_data(granularity, months)
                if df is not None and not df.empty:
                    st.session_state[f'data_{granularity}'] = df
            ...
            if df is not None and not df.empty:
                st.write(f"{timeframe} 데이터 샘플 (최근 순, 한국 시간):")
                st.write(df.head())
        ...
        if trade_history is not None and not trade_history.empty:
            # totalProfits 열이 있고, 그 값이 0이 아닌 행만 필터링
            if 'totalProfits' in trade_history.columns:
                non_zero_profits = trade_history[trade_history['totalProfits'] != 0]
                if not non_zero_profits.empty:
                    st.write("수익/손실이 발생한 거래 내역:")
                    st.dataframe(non_zero_profits)
                    
                    # 거래 통계
                    total_trades = len(non_zero_profits)
                    st.write(f"총 거래 횟수: {total_trades}")
                    
                    profitable_trades = len(non_zero_profits[non_zero_profits['totalProfits'] > 0])
                    loss_trades = len(non_zero_profits[non_zero_profits['totalProfits'] < 0])
                    st.write(f"수익 거래: {profitable_trades}")
                    st.write(f"손실 거래: {loss_trades}")
                    
                    # 누적 수익 차트
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=non_zero_profits.index, y=non_zero_profits['totalProfits'].cumsum(),
                                            mode='lines', name='누적 수익'))
                    fig.update_layout(title='누적 수익 차트', xaxis_title='날짜', yaxis_title='누적 수익')
                    st.plotly_chart(fig)
                else:
                    st.warning("수익/손실이 발생한 거래가 없습니다.")
            else:
